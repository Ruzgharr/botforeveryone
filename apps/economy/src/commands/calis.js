...
const jobs = [
  ...
  { title: "Garson", desc: "Günün en yoğun vardiyasında masalara eksiksiz servis yaptınız ve bahşiş topladınız", min: 60, max: 150 },
...
export default {
  ...
  async execute({ client, message, args, config }) {
    ...
    if (workCooldowns.has(userId)) {
      ...
      if (now < expirationTime) {
        ...
        return message.reply(MessageFormatter.warn(
          "Yorgunsunuz",
          `Tekrar çalışabilmek için **${timeLeftMins} dakika** dinlenmeniz gerekiyor.`
        ));
      }
    }
    ...
    return message.reply(MessageFormatter.success(
      ...
      `▫️ ${job.desc}.\n▫️ Kazancınız: **+${earned.toLocaleString("tr-TR")} Coin**\n▫️ Güncel Cüzdan: **${((eco.wallet || 0) + earned).toLocaleString("tr-TR")} Coin**`
    ));
  }
};
